import defaultsConfig from './configs/defaults';
...
class RedpackRain {
  ...
  public setOptions(options: RedpackRainOptions) {
    if (!this.timer) {
      // 若没有启动红包雨，则无法调用该方法
  ...
  private clickListener = (event: MouseEvent | TouchEvent) => {
    ...
    if (event.type === 'touchstart') {
      // touchstart
      touchClients = Array.prototype.slice.call((event as TouchEvent).touches).map((touch: any) => ({
    ...
    touchClients.forEach(({ clientX, clientY }) => {
      ...
      // eslint-disable-next-line no-restricted-syntax
      for (const key of Object.keys(this.redpackItemList)) {
